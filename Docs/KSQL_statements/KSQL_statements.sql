...
humidity DOUBLE, weightValue DOUBLE, weightUnit VARCHAR) WITH (KAFKA_TOPIC='AGV_1', VALUE_FORMAT='JSON_SR');
...
CREATE TABLE SAFE_FINAL11 AS 
    SELECT 
        s.MACHINEID,
        MAX(s.MACHINEID) AS MACHINE_ID1,
        ...
        FROM AGV_1_STREAM_wu s
            ...
            GROUP BY MACHINEID
            HAVING COUNT(s.TRACTION) > 3
    EMIT CHANGES; 


CREATE TABLE ALARM_F_1 AS 
    SELECT  
        ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99), 
        s.MACHINEID AS MACHINEID,
        COUNT(s.TRACTION) AS ALARM_COUNTER,
        WINDOWEND AS END_TIME
        FROM AGV_1_STREAM_wu s
            WINDOW TUMBLING(SIZE 30 SECONDS)
            WHERE ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99) = 'alarm'
            GROUP BY ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99), s.MACHINEID
            HAVING COUNT(s.TRACTION) > 3
    EMIT CHANGES; 


--    ksql> SELECT s.SAFE_COUNTER, a.ALARM_COUNTER FROM SAFE_FINAL11 s JOIN ALARM_FINAL1 a ON s.MACHINEID = a.MACHINE_ID EMIT CHANGES;
--Cannot repartition a TABLE source. If this is a join, make sure that the criteria uses the TABLE's key column STATE_NAME instead of MACHINE_ID



SELECT s.SAFE_COUNTER, a.ALARM_COUNTER FROM SAFE_FINAL s JOIN ALARM_FINAL a ON s.MACHINE_ID = a.MACHINE_ID EMIT CHANGES;


-- ------------------------  CORRECT ONE
CREATE TABLE ALARM_TF_1 WITH (KEY_FORMAT='JSON')
    AS 
    SELECT     
...
CREATE TABLE SAFE_TF_1 WITH (KEY_FORMAT='JSON')
    AS 
    SELECT     
        MAX(ROWTIME) AS RECORD_TIME, 
        ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99) AS STATE_NAME, 
...
CREATE TABLE SAFE_TT_10 WITH (KEY_FORMAT='JSON')
    AS 
    SELECT     
        MAX(ROWTIME) AS RECORD_TIME, 
        STRUCT(STATE_NAME := ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99), MACHINEID :=  s.MACHINEID) AS K, 
        COUNT(s.TRACTION) AS SAFE_COUNTER,
        WINDOWSTART AS START_TIME,
        WINDOWEND AS END_TIME,
        WINDOWEND - WINDOWSTART AS SESSION_TIME
        FROM AGV_1_STREAM_wu s 
            WINDOW TUMBLING(SIZE 30 SECONDS)
            WHERE ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99) = 'safe'
            GROUP BY STRUCT(STATE_NAME :=  ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99), MACHINEID := s.MACHINEID) 
            HAVING COUNT(s.TRACTION) > 0
    EMIT CHANGES; 


CREATE TABLE ALARM_TT_10 WITH (KEY_FORMAT='JSON')
    AS 
    SELECT     
        MAX(ROWTIME) AS RECORD_TIME, 
        STRUCT(STATE_NAME := ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99), MACHINEID :=  s.MACHINEID) AS K, 
...
-- V3 
CREATE TABLE B_10 WITH (KEY_FORMAT='JSON')
    AS 
    SELECT     
        MAX(ROWTIME) AS RECORD_TIME, 
        STRUCT(STATE_NAME := ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99), MACHINEID :=  s.MACHINEID) AS K, 
        ...
        FROM AGV_1_STREAM_wu s 
            WINDOW TUMBLING(SIZE 30 SECONDS)
            WHERE ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99) != 'none'
            GROUP BY STRUCT(STATE_NAME :=  ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99), MACHINEID := s.MACHINEID) 
            HAVING COUNT(s.TRACTION) > 3
    EMIT CHANGES; 

-- V4
CREATE TABLE STATUS_11 WITH (KEY_FORMAT='JSON')
    AS 
    SELECT     
        MAX(ROWTIME) AS RECORD_TIME, 
        STRUCT(STATE_NAME := ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99), MACHINEID :=  s.MACHINEID) AS K, 
        SUM(CASE WHEN ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99) = 'alarm' THEN 1 ELSE 0 END) AS ALARM_COUNTER,
        WINDOWSTART AS START_TIME,
        WINDOWEND AS END_TIME,
        WINDOWEND - WINDOWSTART AS SESSION_TIME
        FROM AGV_1_STREAM_wu s 
...
CREATE TABLE STATUS_12 WITH (KEY_FORMAT='JSON')
    AS 
    SELECT     
        MAX(ROWTIME) AS RECORD_TIME, 
        STRUCT(STATE_NAME := ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99), MACHINEID :=  s.MACHINEID) AS K, 
        SUM(CASE WHEN ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99) = 'safe' THEN 1 ELSE 0 END) AS SAFE_COUNTER,
        WINDOWSTART AS START_TIME,
        WINDOWEND AS END_TIME,
        WINDOWEND - WINDOWSTART AS SESSION_TIME
        FROM AGV_1_STREAM_wu s 
...
SELECT a.ALARM_COUNTER, s.SAFE_COUNTER, s.RECORD_TIME FROM STATUS_11 a JOIN STATUS_12 s ON a.K = s.K EMIT CHANGES;
...
CREATE TABLE STATUS_10 WITH (KEY_FORMAT='JSON')
    AS 
    SELECT     
        MAX(ROWTIME) AS RECORD_TIME, 
        STRUCT(STATE_NAME := ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99), MACHINEID :=  s.MACHINEID) AS K, 
        SUM(CASE WHEN ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99) = 'alarm' THEN 1 ELSE 0 END) AS SAFE_COUNTER,
        SUM(CASE WHEN ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99) = 'safe' THEN 1 ELSE 0 END) AS ALARM_COUNTER,
        WINDOWSTART AS START_TIME,
        WINDOWEND AS END_TIME,
        WINDOWEND - WINDOWSTART AS SESSION_TIME
        FROM AGV_1_STREAM_wu s 
...
SELECT s.SAFE_COUNTER, a.ALARM_COUNTER FROM SAFE_TABLE_12 s JOIN ALARM_TABLE_12 a ON s.ROWTIME = a.ROWTIME EMIT CHANGES;

SELECT MAX(SAFE_COUNTER) FROM SAFE_COUNT_TABLE WINDOW SESSION ( 30 SECONDS ) GROUP BY SAFE_COUNTER EMIT CHANGES;

SELECT s.SAFE_COUNTER, a.ALARM_COUNTER FROM SAFE_TABLE_12 s JOIN ALARM_TABLE_12 a ON s.STATE_NAME = a.STATE_NAME EMIT CHANGES;


CREATE TABLE SAFE_TABLE_1234 (ROWTIME INT PRIMARY KEY) WITH (KAFKA_TOPIC='SAFE_TABLE_12', PARTITIONS=6, REPLICAS=1);
    AS SELECT   
        MAX(S.ROWTIME) RECORD_TIME,   
        ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', 
        S.TRACTION, 0.99) STATE_NAME,   
        COUNT(S.TRACTION) SAFE_COUNTER,   
        WINDOWSTART START_TIME,   
        WINDOWEND END_TIME,   (
        WINDOWEND - WINDOWSTART) SESSION_TIME 
    FROM AGV_1_STREAM_WU S 
        WINDOW TUMBLING ( SIZE 30 SECONDS )  
    WHERE (ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', S.TRACTION, 0.99) = 'safe') 
    GROUP BY ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90',S.TRACTION, 0.99) 
...
CREATE TABLE BOTH_COUNT_TABLE AS 
SELECT     
    WINDOWSTART AS START_TIME,
    LATEST_BY_OFFSET(WINDOWEND) AS END_TIME,
    MAX(ROWTIME) AS RECORD_TIME, 
    LATEST_BY_OFFSET(ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99)) AS STATE_NAME, 
    COUNT(s.TRACTION) AS DATA_COUNTER
    FROM AGV_1_STREAM_wu s
        WINDOW TUMBLING(SIZE 30 SECONDS)
        WHERE ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99) = 'safe' 
        OR ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99) = 'alarm'

        GROUP BY ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99),
EMIT CHANGES; 

SELECT     
    COUNT_DISTINCT(WINDOWSTART) AS START_TIME,
    WINDOWEND AS END_TIME,
    MAX(ROWTIME) AS RECORD_TIME, 
    LATEST_BY_OFFSET(ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99)) AS STATE_NAME, 
    COUNT(s.TRACTION) AS DATA_COUNTER
    FROM AGV_1_STREAM_wu s
        ...
        GROUP BY ASSIGN_LING_MD('safe:TR_F;20;30;70;80/alarm:TR_F;40;50;80;90', s.TRACTION, 0.99),WINDOWSTART
EMIT CHANGES; 

-- 

SELECT STATE_NAME, COLLECT_LIST(START_TIME), COLLECT_LIST(END_TIME), DATA_COUNTER FROM BOTH_COUNT_TABLE GROUP BY START_TIME, END_TIME, DATA_COUNTER, STATE_NAME  EMIT CHANGES

SELECT  STATE_NAME, 
        SUM(DATA_COUNTER), 
        COLLECT_LIST(START_TIME) AS ST_AGGR, 
        COLLECT_LIST(END_TIME) AS ET_AGGR
    FROM BOTH_COUNT_TABLE
    GROUP BY START_TIME, END_TIME, DATA_COUNTER, STATE_NAME EMIT CHANGES;
